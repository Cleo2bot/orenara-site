    ...
    <article className="border border-bone-line rounded-xs overflow-hidden bg-bone-card flex flex-col group">
      ...
      <Link href={`/diy-kits/${kit.id}`} className="relative w-full aspect-[16/10] overflow-hidden block">
        ...
        <div className="absolute bottom-0 left-0 right-0 px-5 pb-4">
          <p className="font-spec text-[9px] tracking-[0.15em] text-bone/60 uppercase">
            {kit.tagline}
          </p>
        </div>
      </Link>
      ...
      <div className="p-5 flex flex-col flex-1">
          ...
          <div className="flex flex-wrap gap-1.5">
            {kit.lengths.map((l) => (
              <Link
                ...
                className="font-spec text-xs text-ink/65 border border-bone-line bg-bone-tile hover:border-ink/30 hover:text-ink px-2.5 py-1 rounded-xs transition-colors"
              >
                {l}
              </Link>
            ))}
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-col gap-1">
            {kit.idealFor.slice(0, 3).map((use) => (
              ...
              >
                <span className="text-ink/35 mt-0.5 flex-shrink-0">–</span>
                {use}
              </div>
            ))}
          </div>
        ...
        {/* channel note */}
        {kit.channelNote && (
          <p className="text-xs text-ink/50 border-l border-ink/20 pl-3 mb-4 italic leading-relaxed">
            {kit.channelNote}
          </p>
        )}
        ...
        <div className="mt-auto pt-4 border-t border-bone-line space-y-2">
          <button
            ...
            className="w-full bg-ink hover:bg-ink/80 text-bone font-display text-sm tracking-widest uppercase py-3 rounded-xs transition-colors"
          >
            Join waitlist
          </button>
          <Link
    ...
    <div className="min-h-screen bg-bone text-ink">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 border-b border-white/8" style={{ backgroundColor: '#0F1113' }}>
        <div className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
          ...
          <button
            ...
            className="font-spec text-[9px] tracking-widest uppercase text-bone/70 border border-bone/25 hover:border-bone/60 hover:text-bone px-3 py-1.5 rounded-xs transition-colors"
          >
            Join waitlist
          </button>
        </div>
      ...
      <section className="relative min-h-screen flex flex-col justify-end overflow-hidden pt-14">
        ...
        <div className="relative max-w-6xl mx-auto px-6 pb-20 pt-24">
          <p className="font-spec text-xs tracking-widest text-bone/55 uppercase mb-4">
          ...
          <h1 className="font-display text-4xl sm:text-5xl md:text-6xl leading-[1.05] mb-6 max-w-2xl text-bone">
            ...
            <br className="hidden sm:block" /> Configured for your application.
            <br className="hidden sm:block" /> Plug in — no electrician required.
          </h1>
          <p className="text-bone-dim text-lg leading-relaxed max-w-xl mb-10">
            Factory-sealed, fully submersible IP68 strip lighting —
            pre-configured into ready-to-install kits. Pick your length, choose
            your application, and it ships ready to install.
          </p>
          <div className="flex flex-col sm:flex-row gap-3">
            <button
              ...
              className="bg-bone hover:bg-bone/90 text-ink font-display tracking-wider uppercase px-8 py-3.5 rounded-xs transition-colors"
            >
              Find your kit
            </button>
            <button
            ...
            {["IP68 Submersible", "Factory sealed", "240V plug-in", "3yr warranty"].map(
              (t) => (
                <div key={t} className="flex items-center gap-2">
                  <span className="w-1 h-1 rounded-full bg-bone/40 flex-shrink-0" />
        ...
        <div className="max-w-6xl mx-auto px-6 py-16 lg:py-24">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {BOX_ITEMS.map((item) => (
              ...
              >
                <div className="flex-shrink-0 mt-0.5 w-10 h-10 rounded-xs border border-ink/12 bg-ink/5 flex items-center justify-center">
                  <span className="font-spec text-[8px] tracking-wider text-ink/40 uppercase text-center leading-none">
                    {item.tag}
                  </span>
                </div>
                <div>
